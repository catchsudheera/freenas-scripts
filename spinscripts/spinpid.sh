...
##############################################
#
#  Settings sourced from spinpd.config
...
function CPU_check_adjust {
: <<'COMMENT'
Two older methods of checking CPU temp:
### 1 ### CPU_TEMP=$($IPMITOOL sdr | grep "CPU Temp" | grep -Eo '[0-9]{2,5}')
### 2 ###   # Find hottest CPU core
   MAX_CORE_TEMP=0
...
# But we're not using that method
